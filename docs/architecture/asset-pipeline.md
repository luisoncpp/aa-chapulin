...
Case 1 raw art is kept under `tools/raw/case1/` and processed independently so the regenerated scene plates do not disturb shared or later-case assets. The processor exports the Alma Negra bust/full-body poses, seven flat profile icons, six evidence icons, seven 960×540 examine plates, the consult-only corridor plan `plano_pasillo`, and four scene backgrounds, including the day-1-only `bg_clinica_cuarto6.webp` whose baked patient keeps clinic hotspots visible while Examine hides sprites. It also normalizes the inherited `examine_informe_lesiones.webp` plate to the 960×540 detailed-view contract. Pass `--only examine_foto_crimen bg_pasillo_espejo plano_pasillo` to regenerate a subset. Run it before [[verify_assets.py]] whenever Case 1 raw art changes.
...
`examine_plano` intentionally combines a vertical cutaway (3rd floor / 2nd floor / basement) with a top-down Suite 304 detail; keep the `RAMAL 204–304` near the existing center point-target region when redrawing it.
Same chroma pipeline. Sheets in [[tools/raw/]]: Botija, Cecilio, Maruja, Rufino 2×2s plus `maruja_shock_raw.png` and `rufino_breakdown_raw.png` (1×1 full poses). Identity locks: Botija idle (tiny sky-blue flat cap, full black beard, all-black plumber clothes, wrench); Cecilio idle (bottle-bottom glasses, pearl-grey three-piece); Maruja idle (copper-red teased hair, emerald satin, cream stole — never pink); Rufino smug (black tailcoat, ivory vest, white bow tie, gold monocle, waxed mustache — not Aniceto's grey cravat). After slicing, **every** Case 4 `plain` bust runs through `anchor_standing_bust`. Evidence: `case4_evidence_icons_raw.png` is a **4×4** image-only grid; cell (3,3) is unused. It uses the unlabeled-grid extractor, which removes only full-width/height separators and preserves lower artwork; do not apply the labeled-sheet bottom drop box or clipboard, glass, trunk, and note bottoms are clipped. Rewrite icons (`orden_servicios`, `toxicologia_vino`, and several replaced 4×4 cells) also export from 1×1 `*_icon_raw.png` sheets after the grid. The Acta id `foto_crimen` is shared with Case 1, so the hotel polaroid icon is `foto_suite304.webp` (catalog `icon` override). Never overwrite Case 1 `foto_crimen.webp`. The regenerated `foto_suite304.webp` is a deterministic compact polaroid built from a 112×84 cover-crop of `examine_foto_v3.png`, keeping the Court Record thumbnail on the same photo identity lock. `insignia_abogado.webp` stays the Case 1 file. Backgrounds normally cover-crop to 1536×1024: `bg_hotel_lobby`, `bg_hotel_bar`, `bg_hotel_sotano`, `bg_hotel_suite204`, `bg_hotel_cava`, `bg_hotel_azotea`, and the daytime `bg_hotel_azotea_day` variant. The regenerated `bg_hotel_suite` uses `bg_hotel_suite_v5.png` and preserves its near-16:9 source at 1536×864 so both left doors and the right evidence area remain in frame. Its open entrance has a fitted frame, a `304` plaque, an empty latch mortise in the door and an aligned receiver in the right jamb. Deep-examine plates export at 960×540: `examine_foto`, `examine_cadena`, `examine_plano`, `examine_botella`, `examine_nota`, `examine_orden`, `examine_registro`, `examine_sello`; `examine_foto` is sourced from the same wide photo family. Detention / courtroom / `bg_delegacion` / `bg_waiting_room` are reused. See [[tools/case4_hotspot_notes.md]] for painted-object coordinates.

**Regenerate-and-recalibrate rule:** whenever an `examine_*.webp` plate is regenerated or replaced, remeasure every `pointTarget.zones[].bounds` and `detailedView.clickableZones` against the final 960×540 WebP, then update both Spanish and English script data and their geometry tests. Do not assume that the old coordinates survive a redraw: even a small layout shift can move the clickable target away from the painted clue. Review the plate visually before running asset verification.

#### Selective regeneration

Running `python process_case4_assets.py` still regenerates every Case 4 output. To process only selected output stems, pass them positionally or with `--only`:

```powershell
python process_case4_assets.py --only examine_foto foto_suite304
python process_case4_assets.py bg_hotel_suite,botija_idle
```
