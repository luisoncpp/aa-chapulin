...
|---|---|---|---|---|---|
| **D1-T1** | The defendant took the Chicharra. | Sixty seconds later he carried nothing, and the museum was searched piece by piece. | `parte_detencion` | `chipote_chillon` (the "club" is a 200 g squeaky toy) | [[src/case/case1/Private/trial_day1.ts]] |
| **D1-T2** | The blow sounded like "a sackful of iron" from the rubber mallet. | The wound needs a heavy, dense, edgeless, *flexible* object struck downward from behind — a sack of coin, and from above. | `informe_medico` | — (closes on turnabout 1 and the recess) | [[src/case/case1/Private/trial_day1_t2.ts]] |
...
| 2 | What can find the Chicharra here and now? | `antenitas_vinil` |
| 3 | What proves he was inside the gallery? | `rejilla_ducto` |
| 4 | Where did his advance knowledge come from? | `ficha_museo` |

Stage 2 asks for an *instrument*, not a place: phrased as "where is the Chicharra?" it would make `chicharra_oro` — the card for that very piece, sitting in the Acta — the literal answer and punish the player for giving it.
