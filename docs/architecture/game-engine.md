...
- **Case 5 dock camera.** `setStagingCaseId(caseId, trialDay)` in [[src/engine/Private/TrialCaseStaging.ts]] remaps `DON RAMÓN` to `bg_witness.webp` and defaults unposed `DEFENSA` to `chapulin_idle`. It also stores the active trial day, which `isProsecutionTableSpeaker` uses to seat `BERRONDO` at the prosecution table (`bg_courtroom.webp`) on Case 5 day 1, when he is only the prosecution's *coadyuvante*; from day 2 on he is sworn in (day-2 T2, day-3 T3, the day-4 climax) and keeps the witness plate. Cases 0–4 keep Don Ramón at the defense bench. Optional `climax.guiltyDialogue` replaces the generic CULPABLE pair. The complete plate for `case5` shows the episode title and `DIECISIETE MESES` / `SEVENTEEN MONTHS`.
