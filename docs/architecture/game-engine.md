...
- **Case 5 dock camera.** `setStagingCaseId` in [[src/engine/Private/TrialCaseStaging.ts]] remaps `DON RAMÓN` to `bg_witness.webp` and defaults unposed `DEFENSA` to `chapulin_idle`. Cases 0–4 keep Don Ramón at the defense bench. Optional `climax.guiltyDialogue` replaces the generic CULPABLE pair. The complete plate for `case5` shows the episode title and `DIECISIETE MESES` / `SEVENTEEN MONTHS`.
