...
6. **Investigation & Examination Mode** ([[src/engine/Private/InvestigationController.ts#Examine Mode & Tooltips]]):
   ...
   - **Scene Intro Resolution**: opening dialogues trigger only on the first visit or when a conditional `SceneIntro` matches unplayed event flags. On subsequent visits without new events, dialogue is bypassed, the resident character pose is restored from `scene.idlePose` (or matching `SceneIntro.idlePose`, or hidden if `null`), and investigation navigation buttons remain immediately active. Finishing intro, hotspot, or talk dialogue also restores the resident idle pose rather than retaining the last line's speaker sprite.
