...
6. **Investigation & Examination Mode** ([[src/engine/Private/InvestigationController.ts#Examine Mode & Tooltips]]):
   ...
   - `startExamineMode()` enables pointer events on hotspots, attaches a cursor-following tooltip (`#examine-tooltip`), hides the character sprite, and stamps `examine-mode` on `#game-screen` so `#controls-bar` drops onto the 48px examine plate. The bar and examine plate remain `pointer-events: none`; only `.menu-btn` receives clicks, so empty HUD pixels cannot cover floor hotspots. `InvestigationController` tracks pointer coordinates against the plate's screen rect and toggles `examine-hud-hidden` while the pointer is over the plate, regardless of which hotspot is underneath. `Typewriter` suppresses text blips while the dialogue target is not visible.
