...
5. **Visual Effects & Overlays** ([[src/engine/Private/VisualEffects.ts#Dramatic Cut-in Overlays]]):
   ...
   - **Location Fade**: [[src/engine/Private/SceneFade.ts]] covers `#screen-flash` in black. `fadeThroughBlack` swaps the plate while opaque, waits for an asynchronous plate commit when needed, then reveals. Entering trial paints the first intro courtroom shot during the black cover (not after the reveal). Leaving trial for investigation day 2 does the same in reverse: postal background, no bench, no courtroom sprite, `plain` frame. Also used after a Not Guilty so the waiting room is not a hard cut. `fadeToBlack` stays covered for the case-complete plate ([[src/engine/Private/CaseComplete.ts]]) after the last verdict or epilogue line.
