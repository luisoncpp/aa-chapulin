...
| [[docs/flows/present-point-flow.md\|present-point-flow.md]] | Present & Point (Case 4) | Correct present on a statement/stage with `pointTarget`; player clicks a zone on the evidence plate. | [[src/engine/Private/TrialController.ts]], [[src/engine/Private/TrialClimax.ts]] |
| [[docs/flows/evidence-examine-flow.md\|evidence-examine-flow.md]] | Court Record Deep Examine (Case 4) | Player selects evidence with `detailedView` and clicks Examinar Detalle in the Acta. | [[src/engine/Private/ModalManager.ts]], [[src/state/Private/EvidenceCatalogCase4.ts]] |
