...
5. Whatever block runs last before `startTestimony` ends on the witness being called to the stand and a `JUEZ` line ordering the testimony (see [[docs/architecture/case-scripting.md]]). That block is the day intro / opening-present success for the first testimony of a day, and the previous testimony's resolving success dialogue afterwards, so a script edit that appends past the judge's order breaks the hand-off into cross-examination.
