...
Pose names are not state-neutral across cases. In Case 3, `barriga_shock` and `barriga_enojado` are injured wheelchair poses; Case 5 must not reuse them for a healthy witness. Give the healthy family its own keys (`barriga_sorpresa`, `barriga_reclamo`, `barriga_confundido`, `barriga_aliviado`) and verify both the pixels and the case-script references.
