...
## Effective Pattern
Put the current `ClimaxStage.prompt` on the HUD banner and inside the Court Record while the Acta is open for a pending present. Clear both banners when the Acta closes; reopening a pending present can repaint the question from the current stage. Hide it during success dialogue and choice modals so the question does not fight the cutscene. Do not rely on the judge's opening recap as the only UI.
