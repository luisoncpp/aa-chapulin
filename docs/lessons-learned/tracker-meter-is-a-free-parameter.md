...
- 5/4 = 20-step bars (an earlier `truth` was 16 bars of 5/4, `length: 320`; the current one is 4/4).
- 9/8 = 18-step bars.
- 3/4 = 12-step bars.
...
The first attempt at `truth` stayed in 4/4 and tried to imply 5/4 with a 3+3+2 grouping in the bass alone. That reads as syncopation over a square grid, not as an odd meter, because the chords and percussion keep re-asserting the four. If a piece wants an odd meter, every channel has to be written in it. The current `truth` uses 3+3+2 in every channel on purpose, as syncopation inside 4/4 and not as a stand-in for an odd meter.

## What to preserve

When composing in a non-4/4 meter, keep `length` an exact multiple of the bar length so the loop seam lands where the meter expects it. The `truth` test asserts `length` is a multiple of its bar length for exactly this reason — a loop that wraps mid-bar will sound like a dropped beat every time it repeats, which is hard to diagnose by ear.
