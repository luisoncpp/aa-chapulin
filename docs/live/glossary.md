...
| **Presentación desviada (Deflect)** | Presentar una prueba que sí toca la declaración pero todavía no procede. La corte contesta, orienta hacia lo que falta probar y **no cobra penalización**; el jugador vuelve a la misma declaración. Reservado para el razonamiento correcto a destiempo, no para el error. | [[src/types/Private/script.ts#Trial & Testimony Schema]], [[src/case/case5/Private/trial_day3_deflect.ts]] |
...
| **Acta de Personajes (Character Record)** | Second tab of the Court Record, holding one card per person in the case. Visible when the active case declares profiles (Cases 1 and 5). Readable during any cross-examination; presentable only when the court asks for a person by name. | [[src/state/Private/ProfileInventory.ts]], [[src/engine/Private/CourtRecordTabs.ts]], [[docs/flows/character-record-flow.md]] |
...
| **Profesor Jirafales** | Case 1 guest lecturer (investigation only, never takes the stand); also the Case 2 vault owner — the Case 2 trial is presided by the court's usual Judge, and Jirafales only testifies and lends the floor plans in investigation. A neighbor of Don Ramón from the vecindad — so is Doña Florinda, so nobody is surprised they know each other; the greeting is what shames her into letting the defense work. Times everything to the second and corrects El Chapulín's mangled proverbs. | [[src/case/case1/Private/museo.ts]] |
...
| **Toribio Pantoja** | Case 0 defendant: the paletero framed for assaulting Don Nazario; Don Ramón's first defended client, and the first badge the dented insignia ever accredited. | [[docs/specs/case-0-el-primer-juicio-de-monchito.md]] |
| **Don Nazario Cuenca** | Case 0 victim: the rent collector assaulted from behind in the vivienda 4 on 12 July; survives with amnesia of the day. | [[docs/specs/case-0-el-primer-juicio-de-monchito.md]] |
| **Casimiro Lengua** | Case 0 star witness and true culprit; Case 5 victim. A fake "authorized distributor" who spent eleven years cataloguing doors with loose locks, speaks with Berrondo's cadence, and wears a watch on each wrist. | [[docs/specs/case-0-el-primer-juicio-de-monchito.md]], [[docs/specs/case-5-el-tomo-trece.md]] |
...
| **Profesor Jirafales** | Vault owner; hacienda floor plans and generator box are his. The Case 2 trial is presided by the court's usual Judge. | [[src/case/case2/Private/boveda.ts]] |
...
| **Leontina** | Cadena de oro que cruza el chaleco, del ojal al bolsillo del reloj. Objeto y candado de identidad del Lic. Berrondo en el Caso 5. **Sólo Berrondo usa la palabra**; el Chapulín, el Sargento y el narrador dicen «la cadena de oro». Su primera mención en el despacho dispara el chiste de palabra difícil («¿Leon... qué?»). En inglés: *watch chain* para todos, *albert chain* sólo para Berrondo. | [[src/case/case5/Private/despacho_berrondo.ts]], [[tests/case/Case5Vocabulary.test.ts]] |
