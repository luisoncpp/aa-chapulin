...
| **Acta de Personajes (Character Record)** | Second tab of the Court Record, holding one card per person in the case. Visible only when the active case declares profiles — Case 1 today, Case 5 too once it ships. Readable during any cross-examination; presentable only when the court asks for a person by name. | [[src/state/Private/ProfileInventory.ts]], [[src/engine/Private/CourtRecordTabs.ts]], [[docs/flows/character-record-flow.md]] |
