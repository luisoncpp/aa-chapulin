...
| **Doctor Chapatín** | **Acusado** | Médico anciano, gruñón y tacaño. Cabello y bigote blanco despeinado, lentes pequeños sobre la nariz, bufanda al cuello, bata blanca sobre traje y su inseparable **bolsa de papel de estraza** que usa como arma. Se enfurece si le preguntan la edad. Calla su coartada por **secreto profesional**, aunque le cueste la cárcel. Es, además, el mejor perito forense del caso. | `chapatin_idle`, `chapatin_enojado`, `chapatin_bolsa`, `chapatin_sweat`, `chapatin_conmovido` |
...
BARRIGA (barriga_enojado): ¿Cuáles honorarios? ¡Si el doctor no le pagó!
...
|---|---|---|
| **Doctor Chapatín** | `chapatin_idle`, `chapatin_enojado`, `chapatin_bolsa`, `chapatin_sweat`, `chapatin_conmovido` | Anciano bajito, cabello y bigote blanco despeinado, **lentes pequeños redondos sobre la nariz**, **bufanda beige/amarillenta envuelta al cuello** sobre traje oscuro y bata blanca médica. **Bolsa de papel de estraza** siempre sujeta contra el pecho o lista para golpear. La bolsa, los lentes y la bufanda son parte obligatoria de la silueta en las cinco poses. |
| **El Sargento** (Sargento Refugio Pazguato) | `pazguato_idle`, `pazguato_saludo`, `pazguato_sweat`, `pazguato_decidido` | **Fidelidad canónica a Rubén Aguirre (*Los Caquitos*):** extremadamente alto, delgado/espigado y desgarbado (1.96 m en proporción de sprite). Bigote espeso recortado, uniforme policial azul marino/caqui de la Ciudad de México, kepis/gorra de plato torcida, placa metálica, fornitura con tolete y libreta en el bolsillo. Nunca elegante ni corpulento. |
...
| **Señor Barriga** | `barriga_idle`, `barriga_vendado`, `barriga_shock`, `barriga_enojado` | Traje café, portafolios, sombrero. **Lock: `barriga_idle`** (de pie, sin venda). `barriga_vendado`, `barriga_shock` y `barriga_enojado` son **el mismo recorte de silla de ruedas** con venda craneal; sólo cambia la cara. En juicio, clímax y epílogo de este caso no usar `barriga_idle`. |
...
3. **Motor** (`TrialController`): lista visible de declaraciones + `pressedStatementIds` en el snapshot; `TrialDayRouter` recorre la cadena de aplazamientos; `TrialClimax` acepta `ClimaxStage.requiredUpdateStage` (E4: `microfono_oro` exige etapa 2).
