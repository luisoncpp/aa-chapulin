# genoveva_reglamento — hoja de hechos

Clase: `sprite`. Espec: §23.0 y §23.2.C. Guion: presentación de Genoveva, testimonio D4-T1 §17.2 y sus presiones/resoluciones.

Activo compartido por ES/EN: no existe `genoveva_reglamento_en`. La misma imagen se usa en ambos idiomas; sólo cambia el diálogo y la ficha de perfil (`trial_day4_t1.ts:34-109`; `trial_day4_t1_en.ts:34-109`; `ProfileCatalogCase5Es.ts:74-81`; `ProfileCatalogCase5En.ts:74-81`).

## Mapa semántico

- Persona: Genoveva Peñaloza Suárez, encargada de la ventanilla de peritos y auxiliares de la justicia, testigo D4-T1. Es exacta, no hostil, y contesta sólo lo que se le pregunta (spec §3.2, §6.1, §17.2; `ProfileCatalogCase5Es.ts:74-81`; `ProfileCatalogCase5En.ts:74-81`).
...
- [ ] `PINTAR` — Busto hasta la cintura, de frente, con la misma silueta de Genoveva y la carpeta contra el pecho (`docs/specs/case-5-el-tomo-trece.md:4024-4031`).
- [ ] `PINTAR` — Dedo índice derecho apoyado en la carpeta; mentón algo más alto; cejas rectas; boca en gesto de recitar (`docs/specs/case-5-el-tomo-trece.md:4030-4031`).
- [ ] `PINTAR` — Mujer de 38 años, estatura media y porte rígido, con cara ovalada y labios finos (spec §23.2.C).
- [ ] `PINTAR` — Cabello castaño oscuro en chongo bajo muy apretado, sin pelo suelto; anteojos rectangulares de pasta negra (spec §23.2.C).
...
- [ ] `PINTAR` — Reloj pequeño en la muñeca izquierda, lápiz detrás de la oreja derecha y carpeta de hule negro apretada contra el pecho con el brazo izquierdo (spec §23.2.C).
- [ ] `PINTAR` — Dos rectángulos blancos reflejados en los anteojos, como recurso gráfico de estilo y no como textura fotográfica (`docs/specs/case-5-el-tomo-trece.md:4031`).
- [ ] `AUSENTE` — Gota de sudor, mano subiendo a los anteojos, carpeta descendida, ojos enormes, boca pequeña abierta o lápiz cayéndose: esos cambios pertenecen a `genoveva_sweat` y `genoveva_shock` (§23.2.C).
- [ ] `AUSENTE` — Postura hostil, burlona o cómica; Genoveva es exacta y no hostil (spec §3.2, §17.2).

### F2 Texto en imagen

- [ ] `AUSENTE` — No imprimir nombre, reglamento, cifras, etiquetas, frases, marcas de idioma ni texto auxiliar en el sprite. Las afirmaciones se entregan mediante diálogo ES/EN (`trial_day4_t1.ts:34-111`; `trial_day4_t1_en.ts:34-111`).
...
- [ ] Contrato §23.0 completo en el prompt de regeneración: ilustración 2D estilo *Capcom Ace Attorney* de linaje GBA/Nintendo DS en alta definición;  y ; contorno continuo carbón `#1A1A1A`, más grueso en la silueta; anatomía expresiva y exagerada; colores primarios saturados; tema Chespirito/Chapulín Colorado de televisión mexicana de los setenta, melodrama judicial serio, papel, madera, polvo, latón envejecido y luz de tungsteno; prohibidos fotorrealismo, 3D, acuarela, texturas fotográficas, anime moderno, cómic americano, marcas de agua, texto en inglés salvo variantes EN exigidas y cualquier rosa o magenta en primer plano (spec §23.0).
- [ ] `NO CONTRADECIR` — Mantener el candado de identidad de Genoveva; no convertirla en una empleada genérica ni sustituir la carpeta, el chongo, los anteojos, el camafeo, el reloj o el lápiz (spec §23.2.C).
...
- `genoveva_idle`, `genoveva_sweat`, `genoveva_shock` — comparten rostro, edad, peinado, vestuario, accesorios, carpeta y silueta base. `genoveva_idle` es el candado de identidad; regenerar juntos, este activo no es fuente de verdad del otro (spec §23.2.C).
- `profile_perfil_genoveva.webp` / `perfil_genoveva` — comparte la identidad y el oficio de encargada de ventanilla; regenerar juntos, este activo no es fuente de verdad del otro (`ProfileCatalogCase5Es.ts:74-81`; `ProfileCatalogCase5En.ts:74-81`).
- ES↔EN `genoveva_reglamento` — gemelos de idioma de la misma imagen, sin variante visual; regenerar juntos, este activo no es fuente de verdad del otro.
- `libro_peritos` y la carpeta de vales — comparten el contexto narrativo de registro que la pose acompaña, pero no son fuente visual del sprite; regenerar juntos sólo si se rehacen como continuidad de documentos, este activo no es fuente de verdad de ellos (`docs/specs/case-5-el-tomo-trece.md:3125-3129,3190-3194,3243-3248`).
...
- No se inspeccionó ninguna imagen, conforme al protocolo `asset-facts`; no se confirma ningún defecto visual de la hoja actual.
- Hoja derivada únicamente del spec, los catálogos de perfil ES/EN y los scripts ES/EN de D4. No se ejecutaron tests, lint, auditorías, procesamiento ni verificación de assets.
