...
- [ ] `AUSENTE` — Tachaduras, correcciones, columna de salida, columna de permanencia o cualquier elemento que convierta la devolución del gafete en prueba automática de salida (`src/case/case5/Private/trial_day4_success.ts:56-66`; §24.D I1).
...
- [ ] `TEXTO EXACTO` — Encabezado ES de la segunda columna: `HORA DE DEVOLUCIÓN DE GAFETE` (§23.3; `src/case/case5/Private/trial_day2_success.ts:129`; `src/case/case5/Private/trial_day4_success.ts:58-59`).
...
- [ ] `TEXTO EXACTO` — Variante EN: encabezados `TIME OF ENTRY` y `TIME BADGE RETURNED`; Balbuena conserva `5:44 PM / 6:05 PM` (`src/state/Private/EvidenceCatalogCase5EnB.ts:24-29`; `src/case/case5/Private/trial_day4_success_en.ts:54-59`).
...
- [ ] `NO CONTRADECIR` — El registro no es un censo de quién permanece dentro del Archivo (`src/case/case5/Private/trial_day2_success.ts:126-130`; `src/case/case5/Private/trial_day4_success.ts:64-66`).
...
- [ ] `PINTAR` — En D4-T1 se presenta de nuevo contra la afirmación de Genoveva sobre una hora de salida; el encabezado exacto debe resolver la contradicción (`src/case/case5/Private/trial_day4_t1.ts:44-56`; `src/case/case5/Private/trial_day4_success.ts:54-66`; variantes EN equivalentes).
- [ ] `NO CONTRADECIR` — El libro no demuestra que Berrondo saliera a las 16:50 ni que estuviera fuera antes de las 17:02; sólo documenta entrada y devolución (§24.C F3; `src/case/case5/Private/trial_day4_success.ts:37-40`).
...
- [ ] `NO CONTRADECIR` — Priorizar la legibilidad de ambos encabezados sin convertir el documento en una infografía ni añadir realces editoriales (§23.3; `src/case/case5/Private/trial_day2_success.ts:129`; `src/case/case5/Private/trial_day4_success.ts:58-59`).
...
- [ ] `TEXTO EXACTO` — No traducir el encabezado correcto como `TIME OF DEPARTURE`: el catálogo y el guion EN fijan `TIME OF ENTRY` y `TIME BADGE RETURNED` (`src/state/Private/EvidenceCatalogCase5EnB.ts:24-29`; `src/case/case5/Private/trial_day4_success_en.ts:54-63`).
...
- `genoveva_idle` y `genoveva_reglamento` — comparten la identidad de la encargada de la ventanilla; ningún sprite aparece dentro del documento. **Regenerar juntos, este activo no es fuente de verdad del otro** (`src/case/case5/Private/trial_day4_success.ts:47-51`).

## Conflictos abiertos

- La declaración inicial de Genoveva llama «hora de salida» a la segunda columna, mientras §4.1 P4, §23.3, el catálogo y las líneas de éxito fijan `HORA DE DEVOLUCIÓN DE GAFETE` / `TIME BADGE RETURNED` (`src/case/case5/Private/trial_day4_t1.ts:44`; `src/case/case5/Private/trial_day4_t1_en.ts:44`; `src/case/case5/Private/trial_day4_success.ts:56-63`). Es una contradicción narrativa deliberada que este activo resuelve; no rotular la imagen con «salida».
