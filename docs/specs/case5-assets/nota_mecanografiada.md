...
- [ ] `PINTAR` — Cada `s` minúscula original aparece una sola vez dentro de su palabra, media línea por debajo del renglón y medio grado inclinada a la izquierda; no se conserva otra `s` en la línea base ni se añade una `S/s` separada debajo. Es un defecto de la máquina, no un realce didáctico (§23.3, línea 4051; §16.2, líneas 3019-3021).
