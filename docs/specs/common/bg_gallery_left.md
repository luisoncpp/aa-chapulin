# bg_gallery_left — hoja de hechos

Clase: `bg`. Espec: [[docs/specs/artistic-direction.md]] §3. Guion: todavía sin línea que lo estampe. Runtime previsto: `assets/bg_gallery_left.webp`, 1376×768. Encargo del usuario: 2026-09-24.

## Mapa semántico y alcance

Plano de la grada de la **defensa** (muro izquierdo visto desde el pozo), tomado desde arriba del pasillo, del lado de la fiscalía, girando hacia la esquina de la defensa con el juez. Se usa cuando el narrador habla del público. Encuadra sólo lo que está por encima de las cabezas de los abogados, así que no tiene que mostrar a nadie que esté en juicio.

Guía de composición: `tools/gallery_guide/out/left_guide.png` (plana, sin texto, para entregarla al modelo) y `left_annotated.png` (con rótulos). Se regenera con `python -m tools.gallery_guide --preset left`. Las medidas vienen de `tools/gallery_guide/scale.py`, que las deriva de `STAGE_FRAMES` (`src/engine/Private/StageLayout.ts`).

## Hechos a verificar

### F1 Contenido

- [ ] `PINTAR` — Grada lateral izquierda elevada: tres filas escalonadas sobre el lambrín de la defensa, con parapeto macizo de caoba y canto de latón. El parapeto cruza el tercio inferior izquierdo y sube en diagonal hacia la esquina del fondo (guía: de `x≈0, y≈570` a `x≈590, y≈533`).
...
- [ ] `PINTAR` — Juez en la esquina inferior derecha, cortado por el borde: cabeza hacia `x≈1114, y≈584`, busto de unos 110 px, **del mismo tamaño que el público de la primera fila**. El canto de oro del estrado le tapa el vientre, no el pecho (`JUDGE_LIP`).
...
- [ ] `PINTAR` — El estrado mide unos siete anchos de hombro del juez. Aquí sólo se ve su extremo izquierdo; sale del cuadro por la derecha.
...
- `bg_gallery_right` — espejo de esta lámina con la grada de la fiscalía. Mismo parapeto, filas y juez.
...
- `bg_defense` — el lambrín bajo la grada es el mismo que se ve detrás de la defensa.

## Verificación de escala

Con la imagen candidata ya generada sobre la guía:

    python -m tools.gallery_guide --preset left --overlay <candidata>

Escribe `tools/gallery_guide/out/left_overlay_<candidata>.png` con las siluetas y los muebles en contorno magenta. Si una silueta sale por encima del parapeto más de lo que muestra la guía, o la silla del juez es más ancha que su contorno, falla F1.
